...
/*
 * /api/lol/static-data/{region}/{version}/rune
 */
Client.prototype.runes = function runes(options, cb) {
	if (typeof (options) === 'function') {
		cb = options;
		options = undefined;
	}
	...
	var path = format(BASE_PATH, 'static-data/' + this.region,
		STATIC_VERSION, RUNES_PATH);
	var query = { api_key: this.apiKey };
	if (options) {
		['locale', 'version', 'runeListData'].forEach(function (key) {
			if (options[key] !== undefined) {
				query[key] = options[key];
			}
		});
...
/*
 * /api/lol/static-data/{region}/{version}/rune/{id}
 */
Client.prototype.runeById = function runeById(id, options, cb) {
	assert.number(id, 'id');
	if (typeof (options) === 'function') {
		cb = options;
		options = undefined;
	...
	var itemPath = format(RUNE_BY_ID_PATH, id);
	var path = format(BASE_PATH, 'static-data/' + this.region,
		STATIC_VERSION, itemPath);
	var query = { api_key: this.apiKey };
	if (options) {
...
/*
 * /api/lol/static-data/{region}/{version}/realm
 */
Client.prototype.realm = function realm(cb) {
	assert.func(cb, 'callback');
